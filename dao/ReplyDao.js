...
module.exports.create = ({userIdx, reviewIdx, content}) => {
    return pool.queryParam_Parse(SQL_REPLY_INSERT, [userIdx, reviewIdx, content]);
 ...
 * 
 */
const SQL_REPLY_SELECT = `SELECT rp.user_idx AS userIdx, u.name AS userName, rp.content FROM reply rp INNER JOIN user u ON rp.user_idx = u.user_idx WHERE reply_idx = ?`;
 ...
 * 
 */
const SQL_REPLY_SELECT_ALL = `SELECT rp.reply_idx AS replyIdx, rp.user_idx AS userIdx, u.name AS userName, rp.content FROM reply rp INNER JOIN user u ON rp.user_idx = u.user_idx WHERE review_idx = ?`;
