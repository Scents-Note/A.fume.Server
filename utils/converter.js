...
module.exports.inputStrToDBIntOfReview = async ({longevity, sillage, seasonalList, gender, keywordList}) => {
    try{
        ...
        return {
            longevity: longevity? longevityTypeArr.indexOf(longevity)+1 : null,
 ...
 * @returns {Promise<Review>} 
 */
module.exports.DBIntToOutputStrOfReview = async ({longevity, sillage, sumOfBitSeasonal, gender}) => {
    try{
        // seasonalList 변환하기 (비트연산)
        let seasonalStrList = [];
        if (sumOfBitSeasonal) {
 ...
 * @returns {Promise<Review>} 
 */
module.exports.DBIntToOutputIntOfReview = async ({longevity, sillage, sumOfBitSeasonal, gender}) => {
    try{
        // seasonalList 변환하기 (비트연산)
        let seasonalStrList = [];
        if (sumOfBitSeasonal) {
