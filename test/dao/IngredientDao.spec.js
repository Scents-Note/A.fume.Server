...
describe('# ingredientDao Test', () => {
    describe(' # create Test', () => {
        ...
        let seriesName
        before(async () => {
            seriesName = (await pool.queryParam_Parse("SELECT name FROM series"))[0].name;
            await pool.queryParam_None("DELETE FROM ingredient WHERE name = '테스트 데이터'");
        });
        // 성공 케이스
        it(' # success case', (done) => {
            ingredientDao.create({name : '테스트 데이터', englishName : "Test Data", description : "왈라왈라", seriesName}).then((result) => {
                expect(result).gt(0)
                done();
            }).catch((err) => {
                expect(false).true();
    ...
    describe(' # read test', () =>{
        ...
        it(' # success case', (done) => {
            ingredientDao.read(ingredientIdx).then((result) => {
    ...
    describe('# update Test', () => {
        ...
        it('# success case', (done) => {
            ingredientDao.update({ingredientIdx, name:'수정 데이터', englishName:'Update Data'})
            .then((result) => {
                expect(result).eq(1);
                done();
            }).catch((err) => {
        ...
        after(async () => {
            await pool.queryParam_None(`DELETE FROM ingredient WHERE ingredient_idx=${ingredientIdx}`);
        });
    });
    ...
    describe('# delete Test', () => {
        ...
        it('# success case', (done) => {
            ingredientDao.delete(ingredientIdx).then((result) => {
                expect(result).eq(1);
                done();
            }).catch((err) => {
                expect(false).true();
